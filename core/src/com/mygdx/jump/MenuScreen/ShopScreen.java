...
public class ShopScreen extends ScreenAdapter {
    ...
    private void loadAssets(){
        ...
        Texture actor = new Texture(Gdx.files.internal("data/test.png"));
        TextButton.TextButtonStyle tbs = new TextButton.TextButtonStyle();
        tbs.font = font;
        tbs.up = new TextureRegionDrawable( new TextureRegion(actor) );


        button.add(new TextButton("actor1", tbs));
        button.add(new TextButton("actor2", tbs));
        button.add(new TextButton("actor3", tbs));
        button.add(new TextButton("actor4", tbs));
        button.add(new TextButton("actor5", tbs));
        button.add(new TextButton("actor6", tbs));


        for(int i =0;i<button.size;i++)
        {
            stage.addActor(button.get(i));
            button.get(i).setSize(100,100);
            if(i<button.size/2)
            {
                button.get(i).setPosition((240-button.get(i).getWidth())/2, 500-150*i);
                System.out.println((240-button.get(i).getWidth())/2);
            }
            else
            {
                button.get(i).setPosition((720-button.get(i).getWidth())/2, 500-150*(i-3));
            }
        }
